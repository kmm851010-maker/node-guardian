      ...
      <div className="bg-white rounded-2xl w-full max-w-sm overflow-hidden">
        <div className="p-5 text-center space-y-4">
          <p className="font-bold text-lg">
            {phase === 'ready' && t('dice.title')}
            {phase === 'rolling' && t('dice.rolling')}
            {phase === 'result' && t('dice.result')}
          </p>
          ...
          {phase === 'result' && result && (
            <div className="space-y-1">
              ...
              {result.dice_result === 6 && (
                <p className="text-yellow-500 font-semibold text-sm animate-pulse">{t('dice.jackpot')}</p>
              )}
              <p className="text-xs text-muted-foreground">
                {t(`dice.msg${result.dice_result}`)}
              </p>
            </div>
          )}
          ...
          {phase === 'ready' && (
            <div className="bg-gray-50 rounded-xl p-3">
              <p className="text-xs font-semibold text-muted-foreground mb-2">{t('dice.rewardTable')}</p>
          ...
          <p className="text-xs text-muted-foreground">
            {t('dice.remaining', { count: remaining })}
          </p>
        </div>
        ...
        <div className="p-4 pt-0 flex gap-2">
          ...
          {phase === 'result' && remaining > 0 && (
            <button
              ...
              className="flex-1 py-2.5 bg-violet-600 text-white font-bold rounded-xl text-sm"
            >
              {t('dice.rollAgain', { count: remaining })}
            </button>
          )}
          <button
            onClick={onClose}
          ...
          >
            {phase === 'result' && remaining <= 0 ? t('profile.confirm') : t('dice.close')}
          </button>
        </div>
      </div>
    </div>
